...
class DosenController extends Controller
{
    public function index()
    {
        ...
        for ($semester = 1; $semester <= 14; $semester++) {
            ...
            ->leftJoin('Dosen', 'Mahasiswa.nama_doswal', '=', 'Dosen.nama_doswal')
            ->where('Dosen.NIP', $nip)
            ->whereIn('Mahasiswa.status', $statusTidakAktif) // Use whereIn for multiple status conditions
            ->get();

            // Count the number of students for each status
            $jumlahStatus = [];
    ...
    public function showIRS() {
        ...
        ->leftJoin('IRS', 'Mahasiswa.id_mhs', '=', 'IRS.id_mhs')
        ->select('Mahasiswa.*', 'irs_subquery.max_berkas_irs AS irs', 'IRS.persetujuan AS persetujuan_irs', 'IRS.jumlah_sks AS sks', 'IRS.smst_aktif AS semester')
        ->where('Dosen.NIP', $nip)
    ...
    public function showKHS() {
        ...
        ->leftJoin('KHS', 'Mahasiswa.id_mhs', '=', 'KHS.id_mhs')
        ->select('Mahasiswa.*', 'khs_subquery.max_berkas_khs AS khs', 'KHS.persetujuan AS persetujuan_khs', 'KHS.smt_aktif AS semester',
         'KHS.IP_smt AS ips', 'KHS.IP_kumulatif AS ipk', 'KHS.SKS_semester AS sks', 'KHS.SKS_kumulatif AS sksKum')
        ->where('Dosen.NIP', $nip)
        ->where(function ($query) {
    ...
    public function perwalian($NIM)
    {
        ...
        return view('dosen.perwalian', compact('doswal', 'mahasiswa', 'skripsi', 'PKL', 'allKHS', 'allIRS'));
    }
    ...
    public function rekapStatus()
    {
        ...
        foreach ($tahun as $year) {
            ...
            $jumlahMahasiswaTidakAktif[$year] = 0;
            $jumlahMahasiswaCuti[$year] = 0;
            $jumlahMahasiswaMangkir[$year] = 0;
            $jumlahMahasiswaDO[$year] = 0;
            $jumlahMahasiswaUD[$year] = 0;
            $jumlahMahasiswaLulus[$year] = 0;
            $jumlahMahasiswaMD[$year] = 0;
    ...
    public function editIRS(Request $request, $NIM) {
        ...
        $IRS = IRS::where('NIM', $mahasiswa->NIM)->latest('id_irs')->first();
        ...
        $editSKSSemester = $request->input('sks');

        // Update nilai jumlah SKS pada IRS
        $IRS->jumlah_sks = $editSKSSemester;
        $IRS->save();

        // Mengambil data KHS terbaru berdasarkan NIM
        $KHS = KHS::where('NIM', $mahasiswa->NIM)->latest('id_khs')->first();

        if (!$KHS) {
            return redirect()->back()->with('error', 'KHS tidak ditemukan.');
        }

        // Menghitung SKS kumulatif sebelumnya dan SKS semester sebelumnya
        $sks_kumulatif_sebelumnya = $KHS->SKS_kumulatif ?? 0;
        $sks_semester_sebelumnya = $KHS->SKS_semester ?? 0;

        // Update nilai SKS semester pada KHS
        $KHS->SKS_semester = $editSKSSemester;

        // Menghitung dan memperbarui SKS kumulatif baru pada KHS
        $KHS->SKS_kumulatif = ($sks_kumulatif_sebelumnya - $sks_semester_sebelumnya) + $editSKSSemester;
        $KHS->save();
    ...
    public function editKHS(Request $request, $NIM) {
        $mahasiswa = Mahasiswa::where('NIM', $NIM)->first();
        ...
        $sks_kumulatif_sebelumnya = KHS::where('NIM', $mahasiswa->NIM)
        ->latest('id_khs')
        ->value('SKS_kumulatif') ?? 0;
        $sks_semester_sebelumnya = KHS::where('NIM', $mahasiswa->NIM)
        ->latest('id_khs')
        ->value('SKS_semester') ?? 0;
        // Misalnya, jika field pada form adalah 'SKS_semester', maka:
        $KHS->SKS_semester = $editSKSSemester;
        $KHS->SKS_kumulatif = ($sks_kumulatif_sebelumnya - $sks_semester_sebelumnya) + $editSKSSemester;
        $KHS->save();
